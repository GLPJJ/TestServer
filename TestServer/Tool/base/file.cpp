...
bool File::UnZipFile(const char* file,const char* outDir,const char* password)
{
...
void File::ChangeFileDate(const char *filename,uLong dosdate,tm_unz tmu_date)
{
#ifdef WIN32
  HANDLE hFile;
...
void File::CreateDir(const char* pDir,bool isDir)
{
	char dirName[260] = {0};
	if(pDir)
		strcpy(dirName,pDir);
...
void File::GetFileListFromDir(const std::string dir, const std::string pattern, std::vector<std::string> &files)
{
#ifdef WIN32
...
bool File::IsFileExits(const char* filename)
{
	if(!filename)
		return false;
...
void File::MoveFileG(const char* dst,const char* src)
{
	if(dst&&src)
		rename(dst,src);
}

void File::RemoveFile(const char* dst)
{
	if(dst)
		remove(dst);
}
